...
- [x] **Task 1.2**: Add Vault SDK dependencies
...
- [x] **Task 1.3**: Create directory structure
...
- [x] **Task 1.4**: Populate .gitignore
  - File: `.gitignore`
  - Effort: S
  - Dependencies: None
  - Acceptance: .gitignore contains Go-specific patterns

- [x] **Task 1.5**: Create README.md
  - File: `README.md`
  - Effort: S
  - Dependencies: None
...
- [x] Run: `go mod download` - Downloads all dependencies successfully
- [x] Run: `go mod tidy` - No changes needed
...
- [x] **Task 2.1**: Write backend initialization tests FIRST
...
- [x] **Task 2.2**: Implement Backend struct and Factory
  - File: `backend.go`
  - Effort: M
...
- [x] **Task 2.3**: Create stub config path
  - File: `path_config.go`
  - Effort: S
...
- [x] Run: `go vet ./...` - No vet warnings
- [x] Verify: Backend follows identity engine pattern

---

## Phase 3: Configuration Path

- [x] **Task 3.1**: Define Config struct
  - File: `path_config.go`
  - Effort: S
  - Dependencies: Phase 2 complete
  - Acceptance: Config struct with Issuer, SigningKey, DefaultTTL fields

- [x] **Task 3.2**: Write config path tests FIRST
...
- [x] **Task 3.3**: Implement config path handlers
...
- [x] **Task 3.4**: Implement getConfig helper
  - File: `path_config.go`
  - Effort: S
...
- [x] Run: `go test ./... -v` - All tests pass
- [x] Run: `go test -cover ./...` - Coverage > 80%
- [x] Verify: Config CRUD operations work
- [x] Verify: Signing key NOT returned on read (security)
...
- [x] Run: `go test ./... -v` - All tests pass
- [x] Run: `go test -cover ./...` - Coverage > 80%
- [x] Verify: Role CRUD operations work
- [x] Verify: Template validation works
- [x] Verify: Role list operation works

---

## Phase 5: Token Exchange Path

- [x] **Task 5.1**: Write token exchange tests FIRST
...
- [x] Run: `go test ./... -v` - All tests pass
- [x] Run: `go test -cover ./...` - Coverage > 80%
- [x] Verify: Token exchange accepts JWT input
- [x] Verify: Token validation works (signature, expiration)
- [x] Verify: Template processing works
- [x] Verify: Generated JWT is valid and signed

---

## Phase 6: Main Entry Point

- [x] **Task 6.1**: Implement main.go
  - File: `cmd/vault-plugin-token-exchange/main.go`
  - Effort: M
  - Dependencies: Phase 5 complete
  - Acceptance: Uses plugin.ServeMultiplex(), TLS config, error handling

- [x] **Task 6.2**: Test plugin build
...
- [x] Run: `go build -o vault-plugin-token-exchange cmd/vault-plugin-token-exchange/main.go` - Builds successfully
- [ ] Run: `./vault-plugin-token-exchange` - Starts but waits for Vault connection (expected)
- [x] Verify: Binary size is reasonable (< 50MB)
- [x] Verify: No hardcoded secrets or debug code

---

## Phase 7: GitHub Actions CI/CD

- [x] **Task 7.1**: Create GitHub Actions workflow
  - File: `.github/workflows/test.yml`
  - Effort: M
  - Dependencies: Phase 6 complete
  - Acceptance: Workflow with build, test, lint, coverage jobs

- [x] **Task 7.2**: Configure Go setup in workflow
  - File: `.github/workflows/test.yml`
  - Effort: S
  - Dependencies: Task 7.1
  - Acceptance: Uses Go 1.23, caches dependencies

- [x] **Task 7.3**: Add test job
  - File: `.github/workflows/test.yml`
  - Effort: S
  - Dependencies: Task 7.2
  - Acceptance: Runs `go test -v -cover ./...`, uploads coverage

- [x] **Task 7.4**: Add lint job
  - File: `.github/workflows/test.yml`
  - Effort: M
  - Dependencies: Task 7.2
  - Acceptance: Runs golangci-lint and go vet

- [x] **Task 7.5**: Add build job
  - File: `.github/workflows/test.yml`
  - Effort: S
  - Dependencies: Task 7.2
