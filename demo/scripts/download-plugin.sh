...
if [ -f ./build/vault-plugin-token-exchange ]; then
  echo "Plugin binary already exists at ./build/vault-plugin-token-exchange"
...
curl -sfL "$RELEASE_URL" -o ./build/vault-plugin-token-exchange
...
# Make executable
chmod +x ./build/vault-plugin-token-exchange

echo "Plugin downloaded successfully!"
ls -lh ./build/vault-plugin-token-exchange